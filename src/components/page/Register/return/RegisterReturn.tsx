...
import { returnBook } from '../../../../api/book/book-api';
...
export const RegisterReturn = () => {
  ...
  const onSubmit = (data: any) => {
    if (values.userName.value === '') {
      ...
      returnBook(data).then(() => setSuccessModalOpen(true));
    }
  };
    ...
    <>
      <Flex gap="middle" vertical justify={'center'}>
        <Flex justify={'center'} align={'center'} vertical>
          <Avatar
            size={100}
            icon={<AutoStoriesSharpIcon style={{ width: 200, height: 50 }} />}
            style={{ backgroundColor: '#1677ff', marginBottom: '20px' }}
          />
          <div
            style={{
              display: 'flex',
      ...
      >
        <p style={{ textAlign: 'center', marginBottom: '20px' }}>도서가 성공적으로 반납되었습니다!!</p>
      </Modal>
    </>
  );
};
